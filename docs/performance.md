...
| --- | --- |
| The engine declares a VRAM floor above what this GPU has, or routing fell back to CPU | The routing caveat, naming your card, the engine's floor, and the ways around it. Such a card is also *budgeted* as the CPU-class hardware it performs like — it gets `OMNIVOICE_CPU_GENERATE_TIMEOUT_S`, not the accelerated floor |
